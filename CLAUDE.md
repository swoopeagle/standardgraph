...
- **Crosswalk quality scores:** ~67,324 rows (~64.9%) carry a 1–5 quality score (LLM rubric scoring + deterministic exact-match); remainder unscored (`nlp_pass`, ranked by cosine, treated as neutral quality). ~5,028 flagged for review (score 1–2, hidden by default in `map_standard`).

"""Fetch and ingest Tanzania TIE Mathematics curriculum.

Covered system: tz-tie
Sources (TIE PDFs):
  Primary S3-S7 (Gr 3-7):
    https://maktaba.tetea.org/study-aids/primary/Syllabus%20-%20Mathematics%20-%20S3-S7%20-%202019%20(English).pdf
  O-Level F1-F4:
    https://www.tie.go.tz/uploads/documents/sw-1727256902-MATHEMATICS%20SYLLABUS%20-%20O%20level%20final.pdf
  Advanced Level:
    https://www.tie.go.tz/uploads/files/Advanced%20Mathematics.pdf

Grade mapping: S3-S7 → 3-7, F1-F4 → 8-11, A-Level → HS
IDs: TZ_TIE.MATH.{grade}.{hash}
"""
import json
import re
import sqlite3
...
SYSTEM = "tz-tie"
SOURCE_URL = "https://www.tie.go.tz/"
VERIFIED_DATE = date.today().isoformat()
RAW_DIR = DB_PATH.parent / "raw" / "tanzania"
...
EXTRACT_PROMPT = """\
Extract all mathematics learning objectives from this Tanzania TIE curriculum syllabus for {phase_label}.

The syllabus is organized into:
- Topics or strands (e.g. "Numbers and operations", "Algebra", "Geometry", "Trigonometry", "Calculus", "Statistics")
- Specific learning objectives or competencies

Return ONLY a JSON array (no other text, no markdown). Each element must have:
  "topic"        : topic or strand name
  "objective"    : full text of the learning objective

Rules:
- Extract every specific learning objective, competency, or outcome listed.
- Do NOT include pedagogical guidance, teaching methods, or resource recommendations.
- Preserve exact wording.

TIE MATHEMATICS TEXT ({phase_label}):
{text}
"""
...
def _ingest(objectives: list[dict], grades: list[str], conn: sqlite3.Connection, seen_ids: set[str]) -> tuple[int, int]:
    ...
    for obj in objectives:
        ...
        for grade in grades:
            std_id = f"TZ_TIE.MATH.{grade}.{abs(hash(objective_text[:40])) % 100000}"
...
def main() -> None:
    ...
    print("Extracting Tanzania TIE Mathematics standards...")

"""Fetch and ingest South Africa CAPS Natural Sciences and Technology.

Covered system: za-caps-sci
Sources (CAPS PDFs from education.gov.za):
  Intermediate Phase (Gr 4-6):
    https://www.education.gov.za/Portals/0/Documents/Natural%20Sciences%20and%20Technology%20Int.pdf
  Senior Phase (Gr 7-9):
    https://www.education.gov.za/Portals/0/CD/National%20Curriculum%20Statements%20and%20Vocational/CAPS%20SP%20%20NATURAL%20SCIENCES%20GR%207-9%20%20WEB.pdf
  FET Phase (Gr 10-12, Physical Sciences):
    https://www.education.gov.za/Portals/0/CD/National%20Curriculum%20Statements%20and%20Vocational/CAPS%20FET%20_%20Physical%20Sciences%20GR%2010-12%20WEB.pdf

IDs: ZA_CAPS_SCI.SCIENCE.{grade}.{hash}
"""
import json
import re
import sqlite3
...
RAW_DIR = DB_PATH.parent / "raw" / "southafrica_sci"
...
EXTRACT_PROMPT = """\
Extract all science and technology learning objectives from this South Africa CAPS curriculum text for Grade {grade_label}.

The CAPS document organises science into:
- Topics or content areas (e.g. "Matter and Materials", "Organisms and their environment", "Energy", "Forces and motion", "Earth and space")
- Subtopics or specific concepts
...
Return ONLY a JSON array (no other text, no markdown). Each element must have:
  "topic"        : topic or content area (e.g. "Life Processes", "Chemical Bonds")
...
CAPS SCIENCE TEXT (Grade {grade_label}):
{text}
"""
...
def _ingest(objectives: list[dict], grades: list[str], conn: sqlite3.Connection, seen_ids: set[str]) -> tuple[int, int]:
    ...
    for obj in objectives:
        ...
        grade = str(obj.get("grade") or grades[0]).strip()

        for g in grades:
            std_id = f"ZA_CAPS_SCI.SCIENCE.{g}.{abs(hash(obj_text[:40])) % 100000}"
            if std_id in seen_ids:
                continue
            seen_ids.add(std_id)

            grade_band = "9-12" if g == "HS" else None
            conn.execute(
                """INSERT OR REPLACE INTO standards
                   (id, system, subject, grade, grade_band, domain, cluster,
                    standard_text, last_verified_date, source_url)
                   VALUES (?,?,?,?,?,?,?,?,?,?)""",
                (std_id, SYSTEM, "science", g, grade_band,
...
def main() -> None:
    ...
    print("Extracting South Africa CAPS Natural Sciences standards...")

"""Fetch and ingest Uganda NCDC Mathematics curriculum (P1-S4, A-Level).

Covered system: ug-ncdc
Sources (NCDC PDFs from ncdc.go.ug):
  Primary 4 (P4):
    https://ncdc.go.ug/wp-content/uploads/2024/02/P4_MATHEMATICS_SYLLABUS-1.pdf
  Secondary 1-4 (S1-S4):
    https://ncdc.go.ug/wp-content/uploads/2024/02/Mathematics_Syllabus_compressed.pdf
  Advanced Level (A-Level):
    https://ncdc.go.ug/wp-content/uploads/2025/03/PRINCIPAL-MATHS.pdf
  P1-P3, P5-P7: to be found at runtime

Grade mapping: P1-P7 → 1-7, S1-S4 → 8-11, A-Level → HS
IDs: UG_NCDC.MATH.{grade}.{hash}
"""
import json
import re
import sqlite3
...
SYSTEM = "ug-ncdc"
SOURCE_URL = "https://ncdc.go.ug/resource/"
VERIFIED_DATE = date.today().isoformat()
RAW_DIR = DB_PATH.parent / "raw" / "uganda"
...
EXTRACT_PROMPT = """\
Extract all mathematics learning objectives from this Uganda NCDC curriculum syllabus for {phase_label}.

The syllabus is organized into:
- Topics or units (e.g. "Numbers", "Algebra", "Geometry", "Trigonometry", "Calculus", "Statistics")
...
Rules:
- Extract every specific learning objective, competency, or skill listed.
- Do NOT include pedagogical notes, teaching methods, or assessment guidance.
- Preserve exact wording.

NCDC MATHEMATICS TEXT ({phase_label}):
{text}
"""
...
def _ingest(objectives: list[dict], grades: list[str], conn: sqlite3.Connection, seen_ids: set[str]) -> tuple[int, int]:
    ...
    for obj in objectives:
        ...
        for grade in grades:
            std_id = f"UG_NCDC.MATH.{grade}.{abs(hash(objective_text[:40])) % 100000}"
...
def main() -> None:
    ...
    print("Extracting Uganda NCDC Mathematics standards...")
    ...
    print("Note: P1-P3 and P5-P7 materials not yet added.")
    print("Done.")

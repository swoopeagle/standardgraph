...
| Device | Tailscale IP | SSH user | Role |
|---|---|---|---|
| MacBook Pro | 100.118.151.10 | `ianwang` | Development machine |
| Mac Studio | 100.77.63.73 | `ianwangm1max` | Ollama host (LLM inference only) |
...
Mac Studio hosts Ollama exclusively — no standardgraph repo lives there.
...
| DB (as of 2026-06-26) | 158,714 standards, 300 systems | 158,772 standards, 300 systems |
| Last pipeline run | 2026-06-18 (reingest, completed) | 2026-06-18 (reingest, completed) |
| Project path | `~/projects/intl-math-standards-mcp/` | `~/projects/intl-math-standards-mcp/` |

The pipeline logs are named `reingest_YYYYMMDD_HHMMSS.log` (not `overnight_`).
